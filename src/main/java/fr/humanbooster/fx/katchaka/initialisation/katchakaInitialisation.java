...
public class katchakaInitialisation implements CommandLineRunner {
	...
	//DateTimeFormatter dateDeNaissance = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
	//private static LocalDate dateDeNaissance = LocalDate.parse("2020-05-15");
	
	// Default Format: yyyy-MM-ddTmm:HH:ss.SSS
	//LocalDateTime dt1 = LocalDateTime.parse("2011-11-20T13:45:30");
	//private LocalDate dateDeNaissance = LocalDate.parse("2011-11-20");
...
//	private static Faker faker = new Faker(new Locale("fr-FR"));
	...
	private void ajouterPersonnes() {
		if (personneDao.count() == 0) {
			...
			evan.setPseudo("Evan");			
			try {
				evan.setDateDeNaissance(LocalDate.parse("1987-09-26"));
			...
			evan.setEmail("123@hb.com");
			evan.setMotDePasse("123");
			evan.setBio("ma bio");
			evan.setEstFumeur(true);
			...
			try {
				tomate.setDateDeNaissance(LocalDate.parse("1991-05-05"));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			tomate.setEmail("456@hb.com");
			tomate.setMotDePasse("123");
			tomate.setBio("ma bio");
			tomate.setEstFumeur(true);
			...
			try {
				joaquim.setDateDeNaissance(LocalDate.parse("1980-01-01"));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			joaquim.setEmail("utu@hb.com");
			joaquim.setMotDePasse("123");
			joaquim.setBio("ma bio");
			joaquim.setEstFumeur(false);
